...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void sendToAllParticipantsNotification(String username, Event eventForm) {
        ...
        template.convertAndSendToUser(username, "/queue/reply",
                new MessageBroadcast(eventForm.getStart().format(formatter) + " " + eventForm.getTitle()));
    }
    ...
    public void sendToAll(String destination, Event eventForm) {
        ...
        template.convertAndSendToUser(destination, "/queue/reply",
                new MessageBroadcast(eventForm.getStart().format(formatter) + " " + eventForm.getTitle()));
    }
    ...
    public void sendToSpecificUser(List<User> users, Notification notification) {
        for (User it : users) {
            template.convertAndSendToUser(it.getUsername(), "/queue/reply",
                    new MessageBroadcast(notification.getEvent().getTitle()));
        }
    }
